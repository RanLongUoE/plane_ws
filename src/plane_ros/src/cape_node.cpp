...
public:
    ...
    CapeParam(int patch_size);

    // void precompute(int w, int h);

};

CapeParam::CapeParam(int patch_size)
{
    PATCH_SIZE = patch_size;
    calib_path<<"/home/ranlong/dev/plane_extraction/CAPE/Data/tunnel/calib_params.xml";
    loadCalibParameters(calib_path.str(), K_rgb, dist_coeffs_rgb, K_ir, dist_coeffs_ir, R_stereo, t_stereo);
    fx_ir = K_ir.at<double>(0,0);  fy_ir = K_ir.at<double>(1,1);
    cx_ir = K_ir.at<double>(0,2);  cy_ir = K_ir.at<double>(1,2);
    fx_rgb = K_rgb.at<double>(0,0);  fy_rgb = K_rgb.at<double>(1,1);
    cx_rgb = K_rgb.at<double>(0,2);  cy_rgb = K_rgb.at<double>(1,2);
}
...
int callback(const sensor_msgs::ImageConstPtr& msg_colour, const sensor_msgs::ImageConstPtr& msg_depth, CapeParam * param) {
    cv::Mat rgb_img, d_img;
    int width, height;

    rgb_img = cv_bridge::toCvShare(msg_colour)->image;  // 8bit RGB image
    d_img = cv_bridge::toCvShare(msg_depth)->image;   // 16bit depth image

    if(rgb_img.size()!=d_img.size())
    ...
    if (im_count == 0)
        plane_detector = new CAPE(height, width, param->PATCH_SIZE, param->PATCH_SIZE, cylinder_detection, COS_ANGLE_MAX, MAX_MERGE_DIST);
        
    int nr_horizontal_cells = width/param->PATCH_SIZE;
    int nr_vertical_cells = height/param->PATCH_SIZE;
    ...
    for (int r=0;r<height; r++){
        for (int c=0;c<width; c++){
            ...
            X_pre.at<float>(r,c) = (c-param->cx_ir)/param->fx_ir; Y_pre.at<float>(r,c) = (r-param->cy_ir)/param->fy_ir;
        }
    }
    ...
    for (int r=0;r<height; r++){
        int cell_r = r/param->PATCH_SIZE;
        int local_r = r%param->PATCH_SIZE;
        for (int c=0;c<width; c++){
            int cell_c = c/param->PATCH_SIZE;
            int local_c = c%param->PATCH_SIZE;
            cell_map.at<int>(r,c) = (cell_r*nr_horizontal_cells+cell_c)*param->PATCH_SIZE*param->PATCH_SIZE + local_r*param->PATCH_SIZE + local_c;
        }
    }
    ...
    X_t = ((float)param->R_stereo.at<double>(0,0))*X+((float)param->R_stereo.at<double>(0,1))*Y+((float)param->R_stereo.at<double>(0,2))*d_img + (float)param->t_stereo.at<double>(0);
    Y_t = ((float)param->R_stereo.at<double>(1,0))*X+((float)param->R_stereo.at<double>(1,1))*Y+((float)param->R_stereo.at<double>(1,2))*d_img + (float)param->t_stereo.at<double>(1);
    d_img = ((float)param->R_stereo.at<double>(2,0))*X+((float)param->R_stereo.at<double>(2,1))*Y+((float)param->R_stereo.at<double>(2,2))*d_img + (float)param->t_stereo.at<double>(2);

    projectPointCloud(X_t, Y_t, d_img, U, V, param->fx_rgb, param->fy_rgb, param->cx_rgb, param->cy_rgb, param->t_stereo.at<double>(2), cloud_array);
...
int main(int argc, char ** argv){
    ...
    sync.registerCallback(boost::bind(&callback, _1, _2, param));
